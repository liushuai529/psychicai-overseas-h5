...
module.exports = __webpack_require__.p + "img/fail.dfd974d.png";
...
module.exports = __webpack_require__.p + "img/success.3b8b4a6.png";
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "src": __webpack_require__(1367),
      "alt": ""
    }
  }) : _c('img', {
    attrs: {
      "src": __webpack_require__(1366),
      "alt": ""
    }
  }), _vm._v(" "), _c('div', {
    staticClass: "text"
  }, [_vm._v(_vm._s(_vm.hasData ? '訂單支付成功' : '訂單支付失败'))]), _vm._v(" "), _c('div', {
    staticClass: "btn",
    on: {
      "click": _vm.toPage
    }
  }, [_vm._v("\n      " + _vm._s(_vm.hasData ? '點擊查看' : '點擊返回') + "\n    ")])]) : _vm._e()])
},staticRenderFns: []}
...
exports.default = {
  ...
  created: function created() {
    ...
    this.order_id = _utils2.default.getQueryString('order_id');
    this.path = _utils2.default.getQueryString('path');
    this.index = _utils2.default.getQueryString('index'); // 运势订单用
    this.fortune_index = _utils2.default.getQueryString('fortune_index'); // 运势订单用
    this.is_combine = _utils2.default.getQueryString('is_combine'); // 组合测算订单用
    this.combine_type = _utils2.default.getQueryString('combine_type'); // 组合测算类型 1组合三 2组合二
  ...
  methods: {
    query: function query() {
      ...
      this.count++;
      this.loading = true;
      _mintUi.Indicator.open('結果查詢中...');
      (0, _api.getResultAPI)({ order_id: this.order_id }).then(function (res) {
        if (res.data.status === 'PAYED') {
          if (_this.fortune_index == 0 || _this.fortune_index == 1 || _this.fortune_index == 2) {
            try {
              fbq('trackCustom', 'CustomPurchase', {
                product_id: _this.fortune_index == 0 ? 103 : _this.fortune_index == 1 ? 101 : 102
              });
            } catch (err) {
            ...
            tStatistic.send({
              ...
              c_id: 107,
              args: {
                type: _this.fortune_index == 0 ? 1 : _this.fortune_index == 1 ? 2 : 3,
                origin: location.origin,
                channel: localStorage.getItem('suishen_overseas_channel')
              }
            });
          } else if (_this.combine_type) {
            try {
              fbq('trackCustom', 'CustomPurchase', {
                product_id: _this.combine_type == 1 ? 6 : 7
              });
            } catch (err) {
              console.log('no fbq:', err);
            }
          ...
          try {
            var currency = localStorage.getItem('fb_currency');
            var price = localStorage.getItem('fb_price');
            if (currency && price) {
              fbq('track', 'Purchase', { currency: currency, value: price });
          ...
          _this.loading = false;
          _mintUi.Indicator.close();
          (0, _mintUi.Toast)('訂單支付失敗');
        }
      });
    },
    toPage: function toPage() {
      var fbPath = localStorage.getItem('suishen_overseas_fb_path');
      var channel = localStorage.getItem('suishen_overseas_channel');
      if (this.hasData) {
        var url = '';
        // 组合测算
        if (this.is_combine) {
...
var _result = __webpack_require__(723);
...
new _vue2.default({
  ...
  template: '<App/>',
  components: { App: _result2.default }
});
