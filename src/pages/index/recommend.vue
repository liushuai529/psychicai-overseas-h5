...
<template>
    <div class="recommend-container">
        ...
        <div ref="combine" class="combine" :style="{ marginLeft: scrollLeft + 'px', transitionDuration: hasTouch ? '0s' : '0.2s' }">
            <div class="combine-box" :class="{ animation: !hasScroll }">
                ...
                <div class="combine-card" style="margin-right: 0.2rem">
                    ...
                    <div class="combine-top">
                        <div class="combine-info">
                            <div v-if="combineOrder3 && combineOrder3.sub_orders" class="combine-info-left">已成功解锁，快去体验吧</div>
                            <template v-else>
                                <div class="combine-info-left">任选三个享特价<img src="../../assets/img/3.0/combine-price3.png" alt="" /></div>
                    ...
                    <template v-if="!combineLoading">
                        ...
                        <div class="combine-products">
                            <template v-if="showCombine3">
                                <div v-for="(item, index) in combineArr3" class="combine-product" :class="['prod_' + item]" @click="showGroupPopup3(!combineOrder3 || !combineOrder3.sub_orders, 'card')">
                                    <img v-if="!combineOrder3 || !combineOrder3.sub_orders" class="unlock" src="../../assets/img/3.0/unlcok.png" />
                                </div>
                            </template>
                            <template v-else>
                                <img v-for="item in 3" src="../../assets/img/3.0/combine-placeholder.png" class="combine-placeholder" @click="showGroupPopup3(true, 'card')" />
                            </template>
                        </div>
                        <div class="combine-bottom">
                            <!-- 已付款 -->
                            <div class="combine-order" v-if="combineOrder3 && combineOrder3.sub_orders">
                                <div class="combine-order-item" v-for="item in combineOrder3.sub_orders">
                                    <div class="combine-order-btn" :class="{ done: item.order_id }" @click="toResult(item, combineOrder3.order_id)">{{ item.order_id ? "查看结果" : "开始测算" }}</div>
                                    <div class="combine-order-state">{{ item.order_id ? "已测算" : "还未测算" }}</div>
                                </div>
                            </div>
                            <!-- 已选择 -->
                            <template v-else-if="showCombine3">
                                <div class="combine-select" style="margin-bottom: 0.22rem" @click="pay(6)">108RM 立即支付</div>
                                <div class="combine-change" @click="showGroupPopup3(true, 'change')">更改选项</div>
                            </template>
                            <!-- 未选择 -->
                            <div v-else class="combine-select scale" @click="showGroupPopup3(true, 'select')">选择组合</div>
                        </div>
                    </template>
                ...
                <div class="combine-card" style="margin-right: 0.2rem">
                    ...
                    <div class="combine-top">
                        <div class="combine-info">
                            <div v-if="combineOrder2 && combineOrder2.sub_orders" class="combine-info-left">已成功解锁，快去体验吧</div>
                            <template v-else>
                                <div class="combine-info-left">任选两个享特价<img src="../../assets/img/3.0/combine-price2.png" alt="" /></div>
                    ...
                    <template v-if="!combineLoading">
                        ...
                        <div class="combine-products" style="justify-content: center">
                            <template v-if="showCombine2">
                                <div v-for="item in combineArr2" class="combine-product" :class="['prod_' + item]" style="margin: 0 0.12rem" @click="showGroupPopup2(!combineOrder2 || !combineOrder2.sub_orders, 'card')">
                                    <img v-if="!combineOrder2 || !combineOrder2.sub_orders" class="unlock" src="../../assets/img/3.0/unlcok.png" />
                                </div>
                            </template>
                            <template v-else>
                                <img v-for="item in 2" src="../../assets/img/3.0/combine-placeholder.png" class="combine-placeholder" style="margin: 0 0.12rem" @click="showGroupPopup2(true, 'card')" />
                            </template>
                        </div>
                        <div class="combine-bottom">
                            <!-- 已付款 -->
                            <div class="combine-order" style="justify-content: center" v-if="combineOrder2 && combineOrder2.sub_orders">
                                <div class="combine-order-item" style="margin: 0 0.12rem" v-for="item in combineOrder2.sub_orders">
                                    <div class="combine-order-btn" :class="{ done: item.order_id }" @click="toResult(item, combineOrder2.order_id)">{{ item.order_id ? "查看结果" : "开始测算" }}</div>
                                    <div class="combine-order-state">{{ item.order_id ? "已测算" : "还未测算" }}</div>
                                </div>
                            </div>
                            <!-- 已选择 -->
                            <template v-else-if="showCombine2">
                                <div class="combine-select" style="margin-bottom: 0.22rem" @click="pay(7)">88RM 立即支付</div>
                                <div class="combine-change" @click="showGroupPopup2(true, 'change')">更改选项</div>
                            </template>
                            <!-- 未选择 -->
                            <div v-else class="combine-select scale" @click="showGroupPopup2(true, 'select')">选择组合</div>
                        </div>
                    </template>
                    <div v-else class="combine-loading">
                        <img src="../../assets/img/index/loading.png" alt="" />
        ...
        <div class="list">
            <div v-for="item in list" class="card-box" @click="link(item.url, item.item_id)">
                <div class="card">
        ...
        <div class="fb-footer">
            <div class="fb" style="margin-top: 0.36rem; margin-bottom: 0.3rem">
                更多詳細信息請訪問
                <div @click="toFb('https://www.facebook.com/profile.php?id=100086957902199')"><img src="../../assets/img/index/fb.png" alt="" /><span class="link">FB關注[linghit]</span></div>
            </div>
            <div>
        ...
        <!-- 组合抽屉 -->
        <GroupPopup :list="list1" :visible="visible1" :maxNum="3" @update-visible="visible1 = false" @update-check="updateCheck1" @confirm-combine="confirmCombine3"></GroupPopup>
        <GroupPopup :list="list2" :visible="visible2" :maxNum="2" @update-visible="visible2 = false" @update-check="updateCheck2" @confirm-combine="confirmCombine2"></GroupPopup>
        <!-- 支付抽屉 -->
        <PayPopup :visible="visible" :product_id="product_id" :is_combine="true" :combine_ids="combine_ids" @update-visible="visible = false"></PayPopup>
    </div>
</template>

<script>
import { getCombineGoodsListAPI, getCombineOrderAPI, getGoodsListAPI } from "../../api/api";
...
export default {
    ...
    methods: {
        ...
        toResult(item, main_order_id) {
            if (item.order_id) {
                location.href = `${path_enums[item.product_id]}.html#/result?order_id=${item.order_id}`;
            } else {
                // 组合测算跳转详情页
                location.href = `${path_enums[item.product_id]}.html?is_combine=1&main_order_id=${main_order_id}`;
            }
        },
        // 组合测算立即支付
        pay(pid) {
        ...
        showGroupPopup3(flag, from) {
            ...
            if (from === "card") {
                // 测测 三个商品组合卡片选择框的点击
                tStat_10("click", 105, {
                    state: this.showCombine3 ? 2 : 1, // 2已选择商品 1未选择商品
                    origin: location.origin,
                    channel: localStorage.getItem("suishen_overseas_channel"),
                });
            } else if (from === "change") {
                // 测测 三个商品组合卡片更改选项按钮的点击
                tStat_10("click", 106, {
                    origin: location.origin,
                    channel: localStorage.getItem("suishen_overseas_channel"),
                });
        ...
        showGroupPopup2(flag, from) {
            ...
            if (from === "card") {
                // 测测 两个商品组合卡片选择框的点击
                tStat_10("click", 110, {
                    state: this.showCombine2 ? 2 : 1, // 2已选择商品 1未选择商品
                    origin: location.origin,
                    channel: localStorage.getItem("suishen_overseas_channel"),
                });
            } else if (from === "change") {
                // 测测 两个商品组合卡片更改选项按钮的点击
                tStat_10("click", 111, {
                    origin: location.origin,
                    channel: localStorage.getItem("suishen_overseas_channel"),
                });
