...
<template>
    <div class="container">
        ...
        <div class="content" v-if="!loading">
            ...
            <div class="btn" @click="toPage">{{ hasData ? "點擊查看" : "點擊返回" }}</div>
        </div>
    </div>
</template>
        ...
        query() {
            ...
            getResultAPI({ order_id: this.order_id }).then((res) => {
                if (res.data.status === "PAYED") {
                    if (this.fortune_index == 0 || this.fortune_index == 1 || this.fortune_index == 2) {
                        try {
                            fbq("trackCustom", "CustomPurchase", { product_id: this.fortune_index == 0 ? 103 : this.fortune_index == 1 ? 101 : 102 });
                        } catch (err) {
                        ...
                        tStatistic.send({
                            ...
                            c_id: 107,
                            args: {
                                type: this.fortune_index == 0 ? 1 : this.fortune_index == 1 ? 2 : 3,
                        ...
                        try {
                            fbq("trackCustom", "CustomPurchase", { product_id: this.combine_type == 1 ? 6 : 7 });
                        } catch (err) {
                            console.log("no fbq:", err);
                        }
                        ...
                        try {
                            fbq("trackCustom", "CustomPurchase", { product_id: typeMap[this.path] });
                        } catch (err) {
                            console.log("no fbq:", err);
                        }
